...
export async function mergeTrans( req, res ) {
    try {
        const accounts = await accountLogData.getAccountLogs(req).catch(error => console.log(error));
        for (const account of accounts) {
            await transData.mergeTransMoney(account).catch(error => console.log(error));
        }
        const cards = await cardLogData.getCardLogs(req).catch(error => console.log(error));
