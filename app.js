...
app.use(morgan(`:remote-addr  :remote-user [:date[Asia/Seoul]] ":method :url HTTP/:http-version" :status :res[content-length] ":referrer" ":user-agent"`));
...
connectDB()
  .then(() => {
    ...
    try {
      ...
      server = envHttp.createServer(options, app, (req, res) => {
        console.log(`[${config.protocol}] ${config.nodeEnv} - Connceting ...........`);
      }).listen(config.port, () => {
        console.log(`[${config.protocol}] ${config.nodeEnv} - Server is started`);
      });
    } catch (error) {
      console.log(`[${config.protocol}] ${config.nodeEnv} - Server is not Active. Please Check Your Server`);
      console.log(error);
    }
    initSocket(server);
  })
